...
export default function Ticket({ticket, options, history}){
    ...
    const renderTicketAction = () => {
        ...
        switch (ticket.state){
            case "cancel":
                ...
                return <button onClick={cancelTicketHandler} className="btn btn-secondary btn">
    ...
    return (
        <div className="flex justify-between flex-wrap items-center p-3 pl-0 pr-0">
            <p className="font-bold text-neutral">{options[ticket.ticket_option - 1].name} <span className="text-base-content/60">({ticket.ticket_amount}x)</span></p>
            {renderTicketAction()}
        </div>
    );
}